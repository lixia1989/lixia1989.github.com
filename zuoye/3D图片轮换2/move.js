...
function move(obj,josn,options){
	// var timer=null;
	clearInterval(obj.timer);
	options=options||{};
	options.time = options.time||700;
	options.type = options.type||Tween.Bounce.easeOut;
	var iCount = Math.floor(options.time/(1000/60));
	var start={};
	for(var name in josn){
	...
	obj.timer=setInterval(function(){
		n++;
		for(var name in josn){
			var cur = options.type(options.time*n/iCount,start[name],josn[name]-start[name],options.time);
			if(name=='opacity'){
